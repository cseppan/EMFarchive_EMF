...
public class FastDatasetManagerWindow extends AbstractMPSDTManagerTab implements FastDatasetManagerView, Runnable {
    ...
    public FastDatasetManagerWindow(EmfConsole parentConsole, EmfSession session, DesktopManager desktopManager) {
    ...
    private void doLayout(List<FastDatasetWrapper> datasetWrappers, EmfSession session) {

        this.removeAll();

        this.add(createTopPanel(), BorderLayout.NORTH);
        this.add(createTablePanel(datasetWrappers, getParentConsole()), BorderLayout.CENTER);
        this.add(createControlPanel(), BorderLayout.SOUTH);
    }

    private JPanel createTablePanel(List<FastDatasetWrapper> datasetWrappers, EmfConsole parentConsole) {

        this.tableData = new FastDatasetTableData(datasetWrappers);
        this.table = new SelectableSortFilterWrapper(parentConsole, tableData, SORT_CRITERIA);
    ...
    protected JPanel createButtonPanel() {
        ...
        Button newPointButton = new Button("Add Point", new AbstractFastAction(this.getMessagePanel(),
                "Error adding Fast point dataset") {
        ...
        Button newNonPointButton = new Button("Add Non-Point", new AbstractFastAction(this.getMessagePanel(),
        ...
        Button copyButton = new CopyButton(
                new AbstractFastAction(this.getMessagePanel(), "Error copying Fast datasets") {
        ...
        Button controlButton = new Button("Control", new AbstractFastAction(this.getMessagePanel(),
                "Error copying Fast datasets") {
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    ...
                    showError("Error running contols on Fast datasets: " + ex.getMessage());
                }
            }
        });
    ...
    SelectAwareButton editButton(ConfirmDialog confirmDialog) {
        ...
        Action editAction = new AbstractFastAction(this.getMessagePanel(), "Error editing Fast datasets") {
        ...
        SelectAwareButton selectAwareButton = new SelectAwareButton("Edit", editAction, table, confirmDialog);
    ...
    SelectAwareButton viewButton(ConfirmDialog confirmDialog) {
        ...
        Action viewAction = new AbstractFastAction(this.getMessagePanel(), "Error viewing Fast datasets") {
        ...
        return new SelectAwareButton("View", viewAction, this.table, confirmDialog);
    }
            ...
            for (FastDatasetWrapper datasetWrapper : wrappers) {
                ...
                try {
                    ...
                    if (datasetWrapper.isNonPoint()) {
                        FastDatasetView view = new FastNonPointDatasetViewWindow(this.getDesktopManager(), this
                                .getSession(), this.getParentConsole());
                        FastDatasetPresenter presenter = new FastNonPointDatasetViewPresenterImpl(this.getSession(),
                                view, datasetWrapper);
                        presenter.doDisplay();
                    } else {
    ...
    private void createNewPointDataset() throws EmfException {
        ...
        DatasetType datasetType = this.presenter.getDatasetType(DatasetType.orlPointInventory);
        ...
        try {
            ...
            if (view.shouldCreate()) {
                ...
                for (int i = 0; i < inputDatasets.length; i++) {
                    ...
                    if (emfDataset.getDatasetType().getName().equals(DatasetType.orlPointInventory)) {

                        FastDataset fastDataset = new FastDataset();
                        fastDataset.setDataset(emfDataset);
                        fastDataset.setAddedDate(new Date());

                        FastDatasetWrapper fastDatasetWrapper = new FastDatasetWrapper(fastDataset);
    ...
    private void createNewNonPointDataset() throws EmfException {
        ...
        FastDatasetView view = new FastNonPointDatasetCreatorWindow(this.getDesktopManager(), this.getSession(), this
                .getParentConsole());
        FastDatasetPresenter presenter = new FastNonPointDatasetCreatorPresenterImpl(this.getSession(), view);
        presenter.doDisplay();
    }
    ...
    protected void removeSelectedDatasets() throws EmfException {
        ...
        if (datasetWrappers.isEmpty()) {
            ...
            if (size == 1) {
                ...
                message = "Are you sure you want to remove the " + size + " selected datasets?";
            }
    ...
    void copySelectedDatasets() throws EmfException {
        ...
        if (datasetWrappers.isEmpty()) {
            ...
            if (size == 1) {
                ...
                message = "Are you sure you want to copy the " + size + " selected Fast datasets?";
            }
        ...
        if (datasets.isEmpty()) {
            ...
            for (FastDatasetWrapper dataset : datasets) {
                ...
                try {
                    ...
                    showError("Error running contols on Fast datasets: " + e.getMessage());
                }
            }
        }
    }
