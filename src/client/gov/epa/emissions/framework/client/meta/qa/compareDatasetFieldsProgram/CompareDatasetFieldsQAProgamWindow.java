...
public class CompareDatasetFieldsQAProgamWindow extends DisposableInteralFrame implements EditQAEmissionsView {
    ...
    private TextArea joinExpressionsTextField;
    
    private TextArea comparisonExpressionsTextField;
    ...
    private String comparisonExpressions;
    ...
    private JPanel createLayout(EmfDataset dataset) throws EmfException {
        ...
        ScrollableComponent scrollableComment = ScrollableComponent.createWithVerticalScrollBar(this.joinExpressionsTextField);
        scrollableComment.setPreferredSize(new Dimension(450, 105));
        layoutGenerator.addLabelWidgetPair("Join Expressions:", scrollableComment, content);
        ...
        layoutGenerator.addLabelWidgetPair("Comparison Expressions:", scrollableComment2, content);
    ...
    private Action okAction() {
        return new AbstractAction() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (joinExpressionsTextField.getText().trim().length() == 0) {
                    messagePanel.setError("Please specify join expression(s).");
                    return;
                }
                if (comparisonExpressionsTextField.getText().trim().length() == 0) {
                    messagePanel.setError("Please specify comparison expression(s).");
                    return;
                }
...
*/
                ...
                //group by tag
                programArguments.append(EditQAStepWindow.JOIN_EXPRESSIONS_TAG + "\n");
                programArguments.append(joinExpressionsTextField.getText() + "\n");
                //aggregate tag
                programArguments.append(EditQAStepWindow.COMPARISON_EXPRESSIONS_TAG + "\n");
                programArguments.append(comparisonExpressionsTextField.getText() + "\n");
                //match by tag
