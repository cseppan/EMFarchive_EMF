...
//        this.services = services();
    ...
    public void run(User user, SectorScenario sectorScenario, SectorScenarioService service) throws EmfException {
        ...
        try {
//            SectorScenarioTask sectorScenarioTask = new SectorScenarioTask();
//            //factory.create(sectorScenario, user, 
//            //        sessionFactory, dbServerFactory);
//            SectorScenarioRunTask task = new SectorScenarioRunTask( sectorScenarioTask, user, 
//                    services(), service, 
//                    sessionFactory);
//            threadPool.execute(new GCEnforcerTask("Run Strategy: " + sectorScenario.getName(), task));
        } catch (Exception e) {
            log.error("Error running control strategy: " + sectorScenario.getName(), e);
...
//      throw new EmfException("Least Cost Analysis is not supported.");
        ...
        if (!sectorScenario.getCreator().getName().equals(user.getName()))
            throw new EmfException("Only the creator, " + sectorScenario.getCreator().getName() + ", can run the control strategy.");
