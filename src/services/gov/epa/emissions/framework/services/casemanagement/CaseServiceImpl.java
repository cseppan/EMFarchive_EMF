...
//    }
    ...
    public void runJob(CaseJob job, User user, Case jobCase) throws EmfException {
         ...
         * defines the run script, creates a log file, exports the inputs,
        ...
        */
        ...
        if (DebugLevels.DEBUG_6) System.out.println("User: " + user.getName());
        if (DebugLevels.DEBUG_6) System.out.println("Purpose: " + purpose);
        if (DebugLevels.DEBUG_6) System.out.println("Job: " + job.getName());
        if (DebugLevels.DEBUG_6) System.out.println("Job Case: " + jobCase.getName());
        if (DebugLevels.DEBUG_6) System.out.println("Case Inputs null? : " + (inputs==null));

        // pass the inputs to the exportJobSubmitter
        String submitterId = exportService.exportForJob(user, inputs, purpose, job, jobCase);
        
        if (DebugLevels.DEBUG_6) System.out.println("Submitter Id for case job:" + submitterId);
    ...
    public void runJobs(CaseJob[] jobs, User user) throws EmfException {
        ...
        for (CaseJob job : jobs) {
            try {
                ...
                setStatus(localUser, "Job " + job.getName() + " submitted for case " + jobCase + ".",
                        "Run Job");
