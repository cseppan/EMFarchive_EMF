...
//        capIsPoint = checkTableForColumns(emissionTableName(dataset), "plantid,pointid,stackid,segment");
        ...
        sql = "select inv.fips,\n"
                ...
                + "dr.final_emissions * gspro.massfrac as strat_ann_emis,\n"
                + "inv.ann_emis * gspro.massfrac - dr.final_emissions * gspro.massfrac as inv_minus_strat_emis,\n"
                + "case when inv.ann_emis * gspro.massfrac != null then (dr.final_emissions * gspro.massfrac - inv.ann_emis * gspro.massfrac) / inv.ann_emis * gspro.massfrac * 100.0 end as pct_diff_strat_minus_inv,\n"
                + "dr.cm_abbrev,\n"
        ...
        sql += "union all\n"
            ...
            + "dr.final_emissions as strat_ann_emis,\n"
            + "inv.ann_emis - dr.final_emissions as inv_minus_strat_emis,\n"
            + "case when inv.ann_emis != null then (dr.final_emissions - inv.ann_emis) / inv.ann_emis * 100.0 end as pct_diff_strat_minus_inv,\n"
            + "dr.cm_abbrev,\n"
            + "ct.name as control_technology,\n"
            + "sg.name as source_group,\n"
            + "dr.annual_cost,\n"
