...
public class GriddedSummaryEmissionAirQualityResultFileFormat implements FileFormatWithOptionalCols, DelimitedFileFormat {

    protected SqlDataTypes types;

    private FillDefaultValues filler;

    public GriddedSummaryEmissionAirQualityResultFileFormat(SqlDataTypes types) {
        this(types, new FillRecordWithBlankValues());
    }

    public GriddedSummaryEmissionAirQualityResultFileFormat(SqlDataTypes types, FillDefaultValues filler) {
        this.types = types;
        this.filler = filler;
    }
...
//        fsm.sector, fsm.cmaq_pollutant, fsm.inventory_pollutant, x, y, fsm.factor, fsm.transfer_coeff, sum(emis) as emis
        ...
        cols.add(new Column("POPULATION_WEIGHTED_AIR_QUALITY", types.realType(), new RealFormatter()));
        cols.add(new Column("CANCER_RISK_PER_PERSON", types.realType(), new RealFormatter()));
        cols.add(new Column("TOTAL_CANCER_RISK", types.realType(), new RealFormatter()));
        cols.add(new Column("POPULATION_WEIGHTED_CANCER_RISK", types.realType(), new RealFormatter()));
        cols.add(new Column("GRID_CELL_POPULATION", types.realType(), new RealFormatter()));
        cols.add(new Column("PCT_POPULATION_IN_GRID_CELL_TO_MODEL_DOMAIN", types.realType(), new RealFormatter()));
        cols.add(new Column("URE", types.realType(), new RealFormatter()));
