 ...
 * @version $Id: FormattedImporter.java,v 1.1 2005/08/05 13:14:28 rhavaldar Exp $
 */
public abstract class FormattedImporter implements Importer
{
    ...
    /* (non-Javadoc)
     * @see gov.epa.emissions.emisview.io.importer.Importer#importFile(java.io.File, java.lang.String, gov.epa.emissions.emisview.io.importer.FileImportDetails)
     */
    public final String importFile(File file, String dbName, BufferedReader reader, String[] columnNames,
            String[] columnTypes, int[] columnWidths, boolean overwrite) throws Exception
    {
        ...
        if(Constants.SHOW_STATUS)
            System.out.println("Importing table type \"" + tableType
                    + "\" from file " + fileName + " to database " + dbName
                    + " in table " + tableName + " on "
        ...
        acceptor.setDB(dbName);
        //delete table if overwrite
        if(overwrite)
        {
            acceptor.deleteTable(dbName, tableName);
        }
        //else make sure table does not exist
        else if(acceptor.tableExists(dbName, tableName))
        {
            throw new Exception("The table \"" + tableName + "\" already exists. Please select 'overwrite tables if exist' or choose a new table name.");
        }
        ...
        {
            writerFileName = canonicalFileName.substring(0, txtIndex) + ".reimport." + i + canonicalFileName.substring(txtIndex);
        ...
        //show import status
        if(Constants.SHOW_STATUS)
            System.out.println("Added " + numRows
                    + " rows to the database table \"" + tableName
                    + "\" on " + Calendar.getInstance().getTime());
        if(kickOutRows > 0)
            System.out.println("Kicked out " + kickOutRows + " rows to file "
                    + writerFileName);
    ...
    protected final File[] checkFiles(String datasetType, File[] files) throws Exception
    {
        ...
        else
        {
            ...
            for(int i = 0; i < files.length; i++)
            {
                ...
                //if it is a valid file in the first place (binary search doesn't work for null)
                if(tableType != null)
                {
                    int searchIndex = Arrays.binarySearch(tableTypes, tableType);
                    //Valid table type. Check for duplicate file names for table type
                    ...
                    if(searchIndex >= 0 && files[i].getName().toLowerCase().endsWith(".txt"))
                    {
                        ...
                        else
                        {
                            ...
                            throw new Exception("Multiple files for table type \"" + tableType + "\" are not allowed in the same directory");
                        }
                    }
                }
            }
    ...
    /**
     * Override this method to perform any in-line analysis on data, meaning
     * any checks on data which would designate that it should be excluded
     * from insertion into the current table and subsequently kicked out
     * (written) to an 'extra' data file containing other kicked out data.
     * Default behavior is to assume all data lines are valid.
     ...
     * inserted into the table. This is table type specific processing
     * performed after each table is imported, as opposed to dataset type
     * specific processing in postImport() which is performed after ALL tables
     * are imported.
     * Default behavior is to not perform any post processing.
     */
    ...
    protected final void setDataSources(File[] files)
    {
        ...
        java.util.Map/*<TableType, String>*/ dataSources = new java.util.HashMap/*<TableType, String>*/();
...
//            for(File file : files)
    ...
    protected String[] removeSummaryTable(String datasetType, String[] tableTypes)
    {
        ...
        //must send returned array to concrete List constructor in order to use remove method
