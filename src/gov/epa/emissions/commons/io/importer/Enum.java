 ...
 * @version $Id: Enum.java,v 1.1 2005/08/12 15:46:42 rhavaldar Exp $
 */
...
/**
 * Represents a type-safe Enum, similar to C enums. Its main characteristics are:
 * <ul>
 *	 <li>All instances contain a name that should be unique per subclass</li>
 *	 <li>All instances contain a sequential number whose value is depends on
 ...
 *		  works</li>
 *	 <li>Enum instances are comparable based on their
 *		  <code>ordinal</code> value</li>
 *	 <li><code>equals</code> and <code>hashCode</code> are overwritten in a way
 *		  that allows quick comparissons when needed</li>
 * </ul>
 * <p>A subclass may be implemented as below:</p>
 * <pre>
 ...
 * }
 * </pre>
 * <p>Subclasses should be final, as most methods provided in this superclass
 * won't work for subclasses of subclasses.</p>
 * <p>If you find yourself casting the return of
 ...
 * }
 * </pre>
 * <p>There is special support for cases when Enum instances must be, for
 * example, instances of anonymous inner subclasses of a "real" Enum subclass.
 * To get more information on that, check {@link #getBaseClass()}.</p>
 *
 * @author  mister__m
 */
public abstract class Enum implements Comparable, Serializable
{
    ...
    * @param name unique name per subclass.
    ...
    * @exception IllegalArgumentException if there is already an instance of
    *	  with the same name
    */
    ...
    * @param clazz the Enum subclass we want the <code>Map</code> to hold
    *	  instances
    ...
    * @param clazz subclass of Enum whose instances are to be returned
    * @return Collection of available instances
    * @exception NullPointerException if <code>clazz</code> is
    *	  <code>null</code>
    * @exception IllegalArgumentException if <code>clazz</code> is
    *	  not a subclass of <code>Enum</code>
    */
    ...
    * @param clazz a subclass of Enum
    ...
    * @param clazz a subclass of Enum
    * @param name the name of the instance searched
    * @return the searched instance, if it exists; <code>null</code> otherwise
    * @exception NullPointerException if <code>clazz</code> or <code>name</code>
    *	  are <code>null</code>
    * @exception IllegalArgumentException if <code>clazz</code> is
    *	  not a subclass of <code>Enum</code>
    */
    ...
    * valid.
    * @exception NullPointerException if the base class is
    *	  <code>null</code>
    * @exception IllegalArgumentException if the base class is
    *	  not a subclass of <code>Enum</code> or a super class of this instance's
    *	  class
    */
    ...
    *		class of <code>o</code>'s class and if they have the same base class
    */
   private boolean compareClasses(final Object o)
   {
      final Class clazz = getBaseClass();
      return clazz.isAssignableFrom(o.getClass()) &&
      clazz.isAssignableFrom(((Enum)o).getBaseClass());
   }
   ...
   /**
    * Returns the base class of this instance. It is useful when an object is an
    * instance of anonymous inner class that descends from an Enum class. This
    * allows constructions like the following to work:
    *
    * <pre>
    ...
    *   {
    *		 public boolean evaluate(boolean firstCondition,
    ...
    *	 }
    *
    *	 public abstract boolean evaluate(boolean firstCondition,
    *									  boolean secondCondition);
    *
   ...
   /**
    * Overrides equals to make it final. A simple <code>==</code> comparisson is
    * done for testing equality.
    * @param other the other instance to be compared with
    ...
    * @param o another instance of the same class to be compared to
    * @return -1, if <code>o</code> has a ordinal greather than this object's;
    *			 0, if they are the same;
    *			 1, if this instance has a greater value for ordinal than
    *				 <code>o</code>
    * @exception ClassCastException if <code>o</code> is not of the same class
    *	  as <code>this</code>
    */
   public int compareTo(final Object o)
   {
      ...
      return (ordinal == otherOrdinal) ? 0 :
         ((ordinal > otherOrdinal) ? 1 : -1);
   }
    ...
    * @exception InvalidObjectException if the desserizalized instance does not
    *	  exist anymore
    */
