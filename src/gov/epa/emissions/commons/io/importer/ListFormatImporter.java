...
/**
 * This class contains all the common features for importing files
 * which are delimited.
 * 
 * @author Keith Lee, CEP UNC
 * @version $Id: ListFormatImporter.java,v 1.1 2005/08/05 13:14:28 rhavaldar Exp $
 */
public abstract class ListFormatImporter extends FormattedImporter
{
    ...
    private static final String ESCAPE_SEQUENCE_REPLACEMENT = "\\\\";
    ...
    public ListFormatImporter(String splitRegex, boolean useTransactions, boolean multipleOccurances)
    {
        ...
        //TODO have variable for quoteToken; what about those that do not have quote pattern?
    ...
    private String[] parseQuotedText(String[] stringlets, String text, String line) throws Exception
    {
        ...
        {
            throw new Exception("The quoted token " + checkStart
                    + quoteToken + text + quoteToken + checkEnd
                    + " is not properly delimited in line\n" + line);
        }

        //check for index out of bounds
        checkForIndexOutOfBounds(index, stringlets, line);
        //adjust for escape sequences by removing them
        text = text.replaceAll(ESCAPE_SEQUENCE_REGEX, ESCAPE_SEQUENCE_REPLACEMENT);
