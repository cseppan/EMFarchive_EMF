					...
					FROM emissions.' || inv_table_name || ' inv2
						...
						and (
							...
							(
								...
								and 
								strpos('','' || coalesce(inv2.control_measures, '''') || '','', '','' || er2.existing_measure_abbr || '','') > 0 '
								|| case when has_cpri_column then ' or er2.existing_dev_code = inv2.cpri '
								when has_primary_device_type_code_column then ' or er2.existing_dev_code || '''' = inv2.primary_device_type_code '
								else '' end || '
							)
...
--					and source_total.total_ann_emis between coalesce(er.min_emis, -1E+308) and coalesce(er.max_emis, 1E+308)
					...
					and (
						...
						(
							...
							and 
							strpos('','' || coalesce(inv.control_measures, '''') || '','', '','' || er.existing_measure_abbr || '','') > 0 '
							|| case when has_cpri_column then ' or er.existing_dev_code = inv.cpri '
							when has_primary_device_type_code_column then ' or er.existing_dev_code || '''' = inv.primary_device_type_code '
							else '' end || '
						)
...
/*					' || case when include_unspecified_costs = true then '' else '
					...
					inner join (
						...
						FROM emissions.' || inv_table_name || ' inv
							...
							inner join emf.control_measures m
							on m.id = scc.control_measures_id
							and (strpos('','' || coalesce(inv2.control_measures, '''') || '','', '','' || er2.existing_measure_abbr || '','') > 0 '
							|| case when has_cpri_column then ' or er2.existing_dev_code = inv2.cpri '
							when has_primary_device_type_code_column then ' or er2.existing_dev_code || '''' = inv2.primary_device_type_code '
							else '' end || ')
