...
CREATE OR REPLACE FUNCTION public.build_project_future_year_inventory_matching_hierarchy_sql(
	...
	match_type integer	-- 1 = include only Matched Sources, 2 = Include packet records that didn't affect a source, 3 = ?
	) RETURNS text AS
$BODY$
DECLARE
...
BEGIN
	...
	sql :=
		...
		select 
			null::integer as record_id,' || select_columns || ',
		...
		FROM emissions.' || control_program_table_name || ' cp
			' || join_type || ' join emissions.' || inv_table_name || ' inv
			...
			and ' || inv_dataset_filter_sql || ' ' || case when join_type <> 'left outer' then '' else ' and inv.record_id is null' end || '
		...
		where cp.fips is not null 
			...
			and ' || control_program_dataset_version_filter_sql || '
		...
		FROM emissions.' || control_program_table_name || ' cp
			...
			and cp.segment = inv.segment
			and cp.poll = inv.poll
			and ' || inv_dataset_filter_sql || ' ' || case when join_type <> 'left outer' then '' else ' and inv.record_id is null' end || ' ' || case when join_type <> 'left outer' then '' else ' and inv.record_id is null' end || '
